...
// 这个文件封装了原子操作的相关函数。
// 因为跨平台支持的原因这个文件很乱，在linux gcc下多数会进入NGX_HAVE_GCC_ATOMIC宏的分支
...
// 如果参数old等于参数lock, 就将lock置为set, 并返回1；
// 如果参数old不等于参数lock, 就直接返回0。
// __sync_bool_compare_and_swap是gcc内建的原子操作函数
#define ngx_atomic_cmp_set(lock, old, set)                                    \
    __sync_bool_compare_and_swap(lock, old, set)

// 把参数value原子变量的值加上参数add。
// __sync_fetch_and_add是gcc内建的原子操作函数
#define ngx_atomic_fetch_add(value, add)                                      \
