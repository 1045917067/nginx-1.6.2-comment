...
// 这组头文件和实现文件定义了ngx_url_t结构体及相关操作函数,
// 以及地址结构体和地址字符串相互转换的一些函数。
...
// 将一个ip字符串转化成32位int值,如果字符串不是合法ip返回INADDR_NONE(-1)
in_addr_t
...
// 将参数sa转化成字符串形式，如"192.168.1.10:80"或"192.168.1.10"。
// sa[in]: 输入的以sockaddr结构体格式保存的ip和port
// socklen[in]: sa的长度
// text[out]: 用来输出字符串的内存首地址
// len[in]: text所拥有的内存长度，如果转化出得字符串超过这个长度会被截短保存
// port[in]: 不为0转化ip和port, 为0只转化ip
// return: 写入text的长度
size_t
...
// 将代表IPV6或IPV4地址范围的字符串(如2001:0db8::/32、192.168.1.0/24格式)
// 转化成ngx_cidr_t结构体形式。
ngx_int_t
...
ngx_parse_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    // 根据前缀判断地址是unix域、ipv6还是ipv4，然后调用对应解析函数。 
...
// 在确认参数u中的u->url是unix域地址的情况下，根据u->url解析填充整个u结构体。
static ngx_int_t
...
// 在确认参数u中的u->url是ipv4的情况下，根据u->url解析填充整个u结构体。
static ngx_int_t
...
// 在确认参数u中的u->url是ipv6的情况下，根据u->url解析填充整个u结构体。
static ngx_int_t
...
// 根据系统是否支持getaddrinfo()和是否启用ipv6决定使用getaddrinfo()还是gethostbyname()解析host。
...
// 根据参数u中的u->host构建u->naddrs和u->addrs。
ngx_int_t
ngx_inet_resolve_host(ngx_pool_t *pool, ngx_url_t *u)
{
    u_char              *p, *host;
    ...
    // 这个分支的情况是：host部分不是ip, 尝试用host及dns解析这个host字符串，然后用来构建u->addrs和u->naddrs。
    ...
    // 这个分支的情况是：host部分是ip, 直接构建u->addrs和u->naddrs。
        /* MP: ngx_shared_palloc() */
