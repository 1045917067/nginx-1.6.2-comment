...
// 如果name是绝对路径直接返回，如果不是绝对路径加上prefix取得绝对路径。
// pool[in]: 申请内存用的内存池.
// prefix[in]: 如果name是相对路径，以这个prefix做前缀来得到绝对路径。
// name[in&out]: 如果name是绝对路径将不被修改，如果是相对路径将取得的绝对路径给name。
...
// 检测路径是绝对路径还是相对路径
// return: 绝对路径返回NGX_OK， 相对路径返回NGX_DECLINED。
static ngx_int_t
...
//  首先检测slot的路径跟已经添加的路径有没有嵌套
//  如果没有，在cycle->paths添加这个slot对应的元素
ngx_int_t
...
// cache loader 进程在nginx启动时调用这个函数来遍历nginx上次运行留下的cache文件，
// 将过期的cache文件删除，为可用的cache文件建立起索引。
ngx_int_t
