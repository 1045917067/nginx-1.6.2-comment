...
// 这组头文件和实现文件声明了一些字符串相关的结构体实现了一些字符串相关的函数。
...
// 从pool中申请一块内存，并将src中的内容拷贝到新申请的内存里
// 只拷贝内容,不以'\0'结尾
u_char *
...
// dst[in&out]: 如果为NULL,函数功能为src转意为合法html字符串以后增加的长度
//              如果不为NULL，是将src转意为合法html字符串并存入到dst中
// size[in]: src的长度
uintptr_t
...
// 向红黑树插入一个ngx_str_node_t对象节点的回调函数
void
