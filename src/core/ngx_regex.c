...
// 编译正则表达式模版字符串。
// 将正则表达式的pattern转换成PCRE引擎能够识别的结构（struct real_pcre）
// 在解析配置文件期间调用
ngx_int_t
...
// ngx_regex_module模块的init_module回调函数，
// 在解析完配置文件后运行。 
// 对编译后的正则表达式结构进行分析和学习
static ngx_int_t
...
// 给ngx_regex_module配置结构体未配置的成员赋初始值
static char *
