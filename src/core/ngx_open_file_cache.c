...
// 打开一个文件，如果一个文件已经被打开并且使用了缓存fd机制，直接返回缓存的fd
// of[out]: 打开文件的信息
ngx_int_t
...
create:
    ...
    // 当缓存的fd数量超过配置文件设置的最大值时清理一些长期未使用的fd
    if (cache->current >= cache->max) {
        ngx_expire_old_cached_files(cache, 0, pool->log);
    }

    // 没有从内存池中申请内存，因为内存池可能被销毁
...
// 当缓存fd对应的内存池被销毁时清理过期fd，
// fd对应的ngx_cached_open_file_t对象并非从内存池中申请
static void
...
// 当缓存fd对应的内存池被销毁或缓存fd数量大于设置的最大值时调用
// n[in]: 为0，无论有没有过期fd都要删除一个，不为0, 只删除过期的fd 
static void
...
// 将ngx_cached_open_file_t对象插入红黑树的回调函数
static void
...
// 查找某个文件是否被缓存fd,如果被缓存，返回这个文件对应的ngx_cached_open_file_t对象
static ngx_cached_open_file_t *
ngx_open_file_lookup(ngx_open_file_cache_t *cache, ngx_str_t *name,
    uint32_t hash)
{
