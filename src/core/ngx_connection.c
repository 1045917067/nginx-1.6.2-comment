...
// 这个全局变量在linux系统的epoll事件模型下启动后等于ngx_linux_io。
ngx_os_io_t  ngx_io;
...
// 在参数cf->cycle->listening数组中添加参数sockaddr对应的元素。
ngx_listening_t *
...
// 把套接字s封装成ngx_connection_t对象，不会从未占用内存中申请这个ngx_connection_t对象，
// 而是直接从ngx_cycle->free_connections链表里获取。
ngx_connection_t *
...
// 将一个ngx_connection_t对象加入到ngx_cycle->free_connections链表，以备下次重用。
void
...
// 关闭参数c的一些资源，并通过把c挂到ngx_cycle->free_connections使c处于可重用状态。
void
...
// reusable[in]：为1，放入reusable_connections_queue中，
//               为0，从reusable_connections_queue中取出
void
...
// 当free_connections中已经没有ngx_connection_t对象可用时，尝试从reusable_connections_queue中寻找可用连接
static void
...
// 获取参数c绑定到的sockaddr
// c[in&out]: 如果c->local_sockaddr为0，根据c->fd得到c->c->local_sockaddr。
//            输出的地址s根据非0的c->local_sockaddr产生。
// s[out]: 输出的"ip:port"或"ip"字符串。
// port[in]: 如果不为零参数s输出"ip:port"，如果为0参数s输出"ip"。
ngx_int_t
