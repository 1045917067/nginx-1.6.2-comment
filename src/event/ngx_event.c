...
// 所用事件模型的标志位，linux用epoll事件模型下，
// 为NGX_USE_CLEAR_EVENT|NGX_USE_GREEDY_EVENT|NGX_USE_EPOLL_EVENT。
ngx_uint_t            ngx_event_flags;
// 事件相关函数指针的集合。实际使用时调用ngx_add_event等宏定义。
...
//如果没获取到mutex锁，事件模型的最长超时时间。
ngx_msec_t            ngx_accept_mutex_delay;
// 打开ngx_accept_mutex后进行负载均衡操作用的全局变量
...
static ngx_command_t  ngx_event_core_commands[] = {
    ...
    // 每个worker进程的最大连接数，已经被设为deprecated，
    // 因为这个字符串容易被误解是整个程序的最大连接数。
...
{
    // 将ngx_event_timer_alarm设置为1，在epoll_wait()出错返回且发现ngx_event_timer_alarm为1，
...
// 解析配置文件时遇到worker_connections指令时会调用的回调函数
static char *
...
// 创建ngx_event_core_module配置结构体对象 
static void *
...
// 给ngx_event_core_module配置结构体未配置的成员赋初始值
static char *
