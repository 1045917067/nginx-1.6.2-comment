...
static ngx_command_t  ngx_http_core_commands[] = {
    ...
    // 使用sendfile()代替send()，但打开这个选项后将不能使用page cache。
    // 所以大文件时可以打开，同一个文件可能被下载很多次或小文件时不宜打开。
    // 同时，这个选项不能与aio同时配置。
    { ngx_string("sendfile"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_FLAG,
    ...
    // 异步读取文件功能，但打开这个选项后将不能使用page cache。
    // 同时，这个选项不能与sendfile同时配置。
    // 默认不会编译进这个功能
    { ngx_string("aio"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_enum_slot,
    ...
    // 设置一个发送低潮，也就是剩余多少个字节待发送就可以触发epoll可写事件。
    // linux下配置了这个指令也会别忽略
    ...
    // 当请求静态文件不存在时可以返回下一个文件，当下一个不存在仍可以找下一个，以此类推，当都不存在是才返回404。
...
// NGX_HTTP_POST_ACCESS_PHASE阶段的处理函数，
// 如果NGX_HTTP_ACCESS_PHASE阶段把r->access_code置为非零值，这个阶段会把请求结束。
ngx_int_t
...
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    // 申请并初始化这个"location{"字段对应的ngx_http_conf_ctx_t对象
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    // 将clcf->loc_conf指向父层配置loc_conf
    clcf = ctx->loc_conf[ngx_http_core_module.ctx_index];
    clcf->loc_conf = ctx->loc_conf;

    // 根据location指令内容给clcf->name,clcf->exact_match,clcf->noregex,clcf->named赋值
    value = cf->args->elts;

    if (cf->args->nelts == 3) {
    // 有三个参数，即写成"location ^~ /path {"时的情况
        len = value[1].len;
        mod = value[1].data;
        name = &value[2];
...
// 解析配置文件listen参数的回调函数.
// 这个函数的主要流程是构造一个ngx_http_listen_opt_t对象,
// 这个对象为参数调用ngx_http_add_listen函数
static char *
...
// 解析配置文件遇到"root"或"alias"指令时会调用的回调函数
static char *
