...
// 解析配置文件遇到"http{"指令时会调用的回调函数
// 这个函数包含了http框架初始化的全流程
static char *
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    // 申请这个"http{"指令对应的ngx_http_conf_ctx_t对象。
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }

    // 相当于ngx_cycle->conf[ngx_http_module.index] = ctx。
    *(ngx_http_conf_ctx_t **) conf = ctx;


    /* count the number of the http modules and set up their indices */
    // 初始化ngx_http_max_module全局变量，
    // 给每个http模块对应的ngx_module_t对象的ctx_index成员赋值。
     ...
     */
    // 调用每个NGX_HTTP_MODULE类型模块的create_main_conf, create_srv_conf和create_loc_conf回调函数。
    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != NGX_HTTP_MODULE) {
            continue;
        }
    ...
    // 调用每个NGX_HTTP_MODULE类型模块的preconfiguration回调函数。
    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != NGX_HTTP_MODULE) {
            continue;
        }
    ...
    // 为main, srv和loc层配置没有配置的指令赋初始值，同时进行一些其他解析完配置文件的初始化工作。
    // 即调用每个http{}的init_main_conf()回调函数，调用每个server{}的merge_srv_conf()回调函数，
    // 调用每个location{}的merge_loc_conf()回调函数。
    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != NGX_HTTP_MODULE) {
            continue;
        }
        ...
        // 调用每个NGX_HTTP_MODULE类型模块的init_main_conf回调函数。
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        // 给cscf->named_locations和cscf->regex_locations赋值
        if (ngx_http_init_locations(cf, cscfp[s], clcf) != NGX_OK) {
            return NGX_CONF_ERROR;
        }
        // 给pclcf->static_locations赋值
    ...
    // 初始化cmcf->phases数组。只初始化可以被介入自定义函数的7个阶段
    if (ngx_http_init_phases(cf, cmcf) != NGX_OK) {
        return NGX_CONF_ERROR;
    }

    // 初始化cmcf->headers_in_hash散列表。
    if (ngx_http_init_headers_in_hash(cf, cmcf) != NGX_OK) {
        return NGX_CONF_ERROR;
    }


    // 调用每个NGX_HTTP_MODULE类型模块的postconfiguration回调函数。
    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != NGX_HTTP_MODULE) {
            continue;
        }
    ...
    // *cf在上面解析配置文件的过程中已经被改变，将其还原回去。
    *cf = pcf;


    // 根据cmcf->phases初始化cmcf->phase_engine。
    ...
    // 构造监听端口与server间的关联关系，设置http监听端口连接事件的回调函数。
...
// 根据全局变量ngx_http_headers_in初始化cmcf->headers_in_hash散列表。
static ngx_int_t
...
// 给参数module对应的模块在server和location层未配置的指令付初始值。
// 通过调用NGX_HTTP_MODULE类型模块的merge_srv_conf()和merge_loc_conf()回调函数实现。
static char *
    ...
    ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        if (module->merge_loc_conf) {
            ...
            // 把main层的location配置merge到server层的location配置。
...
// 给参数locations链表中每个ngx_http_core_loc_conf_t对象未配置的指令付初始值。
// 通过调用NGX_HTTP_MODULE类型模块的merge_loc_conf()回调函数实现。
static char *
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
    ...
    // 将locations按类别和名字。为取出named和regex类型的clcf作准备。
...
// 生成clcf参数对应的ngx_http_location_queue_t节点对象，并将这个对象加入到locations参数中。
ngx_int_t
...
// 将配置文件配置的端口和地址信息放到cycle->listening数组中
static ngx_int_t
...
// 由ngx_http_optimize_servers函数调用。
static ngx_int_t
...
// 根据参数addr向ngx_cycle->listening中添加元素，并对添加的元素进行初始化。
...
ngx_http_add_listening(ngx_conf_t *cf, ngx_http_conf_addr_t *addr)
{
    ...
    // 根据参数addr向ngx_cycle->listening中添加元素，并对这个元素做一部分初始化。
    // 将添加的ngx_listening_t对象返回，下面代码继续做初始化。
